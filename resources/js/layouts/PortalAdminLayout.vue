...
<template>
    ...
    <div class="min-h-screen bg-white dark:bg-slate-900 font-sans selection:bg-[#2563EB] selection:text-white transition-colors duration-300">
        ...
        >
            ...
            <!-- Theme Toggle Row -->
            <div class="shrink-0 border-t border-slate-100 dark:border-slate-800 px-4 py-2.5">
                <div v-if="!sidebarCollapsed" class="flex items-center justify-between">
                    <span class="text-[12px] font-semibold text-slate-500 dark:text-slate-400">Mode Tampilan</span>
                    <ThemeTogglerButton
                        v-model="activeTheme"
                        variant="ghost"
                        size="default"
            ...
            <div class="relative shrink-0 border-t border-slate-100 dark:border-slate-800 p-2">
                <button
                    type="button"
                    @click="userMenuOpen = !userMenuOpen"
                    :class="[
                        'flex items-center rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-all duration-200 text-left group w-full',
                        sidebarCollapsed ? 'w-10 h-10 mx-auto justify-center px-0' : 'pl-1.5 pr-2 py-1.5'
                    ]"
                ...
                >
                    ...
                    >
                        <div class="flex-1 min-w-0">
                            <p class="text-[13px] font-bold text-slate-900 dark:text-slate-100 truncate leading-snug">{{ userName }}</p>
                            <p class="text-[11.5px] text-slate-400 dark:text-slate-500 truncate leading-snug">{{ userEmail }}</p>
                        </div>
                        <ChevronsUpDown class="w-3.5 h-3.5 text-slate-300 group-hover:text-slate-500 dark:text-slate-500 transition-colors shrink-0 ml-1.5" />
                    </div>
                </button>
                ...
                <transition enter-active-class="transition duration-100 ease-out" enter-from-class="transform scale-95 opacity-0" enter-to-class="transform scale-100 opacity-100" leave-active-class="transition duration-75 ease-in" leave-from-class="transform scale-100 opacity-100" leave-to-class="transform scale-95 opacity-0">
                    <div
                        v-if="userMenuOpen"
                        class="absolute bottom-full left-2 right-2 mb-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-xl p-1.5 z-50 text-xs font-medium space-y-0.5"
                    >
                        <div class="px-2.5 py-2 border-b border-slate-100 dark:border-slate-800 mb-1">
                            ...
                            <p class="text-[10.5px] text-slate-400 dark:text-slate-400 truncate">{{ userEmail }}</p>
                        </div>

                        <Link
                            href="/dashboard"
                            class="flex items-center gap-2 px-2.5 py-2 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-700 dark:text-slate-200 transition-colors"
