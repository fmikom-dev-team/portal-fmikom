...
const handleSearch = () => {
    if (searchQuery.value.trim()) {
        router.get("/trace/admin/alumni", { search: searchQuery.value }, { preserveState: true });
    }
};
    ...
    <AppShell variant="sidebar">
        ...
        <Sidebar collapsible="icon">
            ...
            <SidebarHeader class="flex flex-row items-center justify-between p-5 group-data-[collapsible=icon]:px-0 group-data-[collapsible=icon]:py-4 group-data-[collapsible=icon]:justify-center border-b border-slate-50 dark:border-zinc-900 group/header relative">
                <Link href="/trace/admin" class="flex items-center gap-2.5 shrink-0 transition-all duration-300 group-data-[collapsible=icon]:group-hover/header:opacity-0 group-data-[collapsible=icon]:group-hover/header:scale-75">
                    <div class="flex h-8 w-8 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-blue-700 shadow-md shadow-sky-500/20">
                    ...
                    <div class="flex flex-col text-left leading-none group-data-[collapsible=icon]:hidden">
                        ...
                        <span class="text-[9px] font-semibold uppercase tracking-widest text-sky-600 dark:text-sky-400 mt-0.5">Panel Pengelola</span>
                    </div>
                </Link>
                <SidebarTrigger class="sidebar-trigger-btn shrink-0 bg-transparent text-slate-400 hover:text-sky-600 rounded-lg p-1 w-7 h-7 hover:bg-slate-50 dark:hover:bg-zinc-900 border-none shadow-none transition-all duration-305 group-data-[collapsible=icon]:absolute group-data-[collapsible=icon]:inset-0 group-data-[collapsible=icon]:m-auto group-data-[collapsible=icon]:opacity-0 group-data-[collapsible=icon]:pointer-events-none group-data-[collapsible=icon]:group-hover/header:opacity-100 group-data-[collapsible=icon]:group-hover/header:pointer-events-auto" />
            </SidebarHeader>
                        ...
                        <SidebarMenuItem v-for="item in group.items" :key="item.href" class="group-data-[collapsible=icon]:w-full group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center">
                            <SidebarMenuButton
                                ...
                                :class="item.match($page.url) 
                                  ? 'font-bold text-sky-700 bg-sky-50 dark:bg-sky-950/40 dark:text-sky-400 shadow-sm border border-sky-200/40 dark:border-sky-800/25' 
                            ...
                            >
                                <Link :href="item.href">
                                    <component :is="item.icon" class="h-[18px] w-[18px] shrink-0 transition-colors" 
                                                :class="item.match($page.url) ? 'text-sky-700 dark:text-sky-400' : 'text-slate-400 group-hover:text-slate-650'" />
                ...
                <SidebarMenu>
                    <SidebarMenuItem class="group-data-[collapsible=icon]:w-full group-data-[collapsible=icon]:flex group-data-[collapsible=icon]:justify-center">
                        <SidebarMenuButton
                            ...
                            class="h-10 rounded-xl transition-all duration-200 font-medium text-slate-500 hover:text-sky-700 hover:bg-sky-50/50 dark:text-zinc-400 dark:hover:text-sky-400 dark:hover:bg-sky-950/20"
                        >
                            <Link href="/dashboard">
        ...
        <AppContent variant="sidebar" class="overflow-x-hidden pb-24 md:pb-0 relative z-[1]">
            ...
            <header class="sticky top-0 z-20 flex h-14 shrink-0 items-center justify-between border-b border-slate-100 bg-white px-4 dark:border-slate-800 dark:bg-slate-900 sm:px-6">
                ...
                <div class="flex items-center gap-3">
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <template v-else>
                            <span class="rounded-lg bg-sky-50 px-2.5 py-1 text-[11px] font-black uppercase tracking-wider text-sky-700 dark:bg-sky-950/40 dark:text-sky-400">
                                TRACE
                            </span>
                ...
                <div class="flex items-center gap-2">
                    <!-- Search bar (desktop) -->
                    <div class="relative hidden h-9 w-[180px] items-center rounded-xl border border-slate-200 bg-slate-50 px-3 focus-within:border-sky-400 focus-within:ring-1 focus-within:ring-sky-200 dark:border-slate-700 dark:bg-slate-800 dark:focus-within:border-sky-600 sm:flex xl:w-[220px]">
                        <Search class="h-3.5 w-3.5 shrink-0 text-slate-400" />
                        <input
                            v-model="searchQuery"
                            @keyup.enter="handleSearch"
                            placeholder="Cari alumni, mitra..."
                            class="h-full w-full border-none bg-transparent pl-2 text-[12.5px] font-medium text-slate-700 placeholder-slate-400 focus:outline-none focus:ring-0 dark:text-slate-200"
                        />
                    </div>
                    ...
                    <DropdownMenu>
                        <DropdownMenuTrigger class="flex items-center gap-2 py-1 pl-2 pr-3 rounded-xl outline-none hover:bg-slate-50 dark:hover:bg-zinc-900/60 border border-transparent hover:border-slate-100 dark:hover:border-zinc-850 transition-all duration-300 group">
                            <Avatar class="h-[32px] w-[32px] overflow-hidden rounded-full ring-2 ring-sky-100 dark:ring-sky-900 shadow-sm transition-all duration-300 group-hover:ring-sky-500/30">
                                <AvatarImage v-if="user?.avatar" :src="user.avatar" :alt="user.name" class="object-cover" />
                                <AvatarFallback class="rounded-full bg-sky-100 dark:bg-sky-900 text-sky-700 dark:text-sky-300 text-[10px] font-bold">
                            ...
                            <div class="hidden flex-col text-left leading-none sm:flex">
                                ...
                                <span class="text-[9px] font-semibold text-sky-600 dark:text-sky-400 mt-0.5 uppercase tracking-wider">Pengelola</span>
                            </div>
                        ...
                        <DropdownMenuContent class="w-56 rounded-xl border border-gray-100 p-2 shadow-xl dark:border-zinc-800 dark:bg-zinc-900" align="end" :side-offset="8">
                            <div class="border-b border-slate-100 px-3 py-2 dark:border-slate-700">
                                ...
                                <span class="mt-1 inline-block rounded-md bg-sky-100 px-1.5 py-0.5 text-[9px] font-black uppercase tracking-wider text-sky-700 dark:bg-sky-950 dark:text-sky-400">
                                    Pengelola TRACE
                                </span>
                            </div>
                            <DropdownMenuGroup class="mt-1">
                                <DropdownMenuItem :as-child="true">
                                    <Link class="flex w-full cursor-pointer items-center px-3 py-2 text-xs font-medium text-slate-600 hover:text-sky-600 dark:text-slate-350 dark:hover:text-sky-400" href="/settings/profile">
            ...
            <main v-else class="trace-content flex-1 p-4 transition-colors duration-300 sm:p-6 lg:p-8">
                ...
                <div v-if="isPageLoading" class="animate-pulse space-y-5">
                    ...
                    <div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
                        <div class="col-span-2 h-72 rounded-2xl bg-slate-200 dark:bg-slate-800" />
            ...
            <footer class="shrink-0 border-t border-slate-100 px-6 py-3 dark:border-slate-800">
                <p class="text-center text-[11px] text-slate-400 dark:text-slate-600">
                    TRACE Admin Panel — Tracking & Record Assessment System · FMIKOM · {{ new Date().getFullYear() }}
                </p>
            </footer>
        </AppContent>
    </AppShell>
