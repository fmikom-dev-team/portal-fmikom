...
export function useMapViewport() {
	...
	const updateViewportStats = (
		...
		const visible = allMarkers.filter(
			(m) => layerVisibility[m.category] && bounds.contains(m.latlng),
		);
		...
		visible.forEach((m) => {
			...
			if (m.data.sektor_industri)
				sektorMap.set(
					m.data.sektor_industri,
					(sektorMap.get(m.data.sektor_industri) || 0) + 1,
				);
			if (m.data.nama_kota)
				cityMap.set(m.data.nama_kota, (cityMap.get(m.data.nama_kota) || 0) + 1);
			if (m.data.instansi)
				instansiMap.set(
					m.data.instansi,
					(instansiMap.get(m.data.instansi) || 0) + 1,
				);
			if (m.data.angkatan)
				angkatanMap.set(
					m.data.angkatan,
					(angkatanMap.get(m.data.angkatan) || 0) + 1,
				);
		});
		...
		viewportStats.value = {
			...
			angkatanData: Array.from(angkatanMap.entries())
	...
	const setupViewportTracking = (
		...
		const debouncedUpdate = () => {
			...
			moveEndTimer = setTimeout(() => {
				...
				setTimeout(() => {
					isViewportUpdating.value = false;
				}, 600);
			}, 400);
		};
