...
export function useMapRadius() {
    ...
    const placeRadius = (
        ...
        const count = allMarkers.filter(mk =>
            layerVisibility[mk.category] && center.distanceTo(mk.latlng) <= radiusMeters
        ).length;
    ...
    const updateRadiusSize = (
        ...
        radiusKm.value = km;
        if (radiusCenter.value) placeRadius(rawMap, radiusCenter.value, allMarkers, layerVisibility);
    };
