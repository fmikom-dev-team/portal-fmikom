...
const HeroGallery = defineAsyncComponent(() => import("@/components/Portal/HeroGallery.vue"));
const MasonryGallery = defineAsyncComponent(() => import("@/components/Portal/MasonryGallery.vue"));
const Testimonials = defineAsyncComponent(() => import("@/components/Portal/Testimonials.vue"));
const AlumniMap = defineAsyncComponent(() => import("@/components/Portal/AlumniMap.vue"));
...
const dismissBanner = () => {
	if (props.pinned_announcement) {
		localStorage.setItem("dismissed_announcement_id", String(props.pinned_announcement.id));
	}
...
const updateNewsScrollState = () => {
	...
	requestAnimationFrame(() => {
		if (!newsScrollContainer.value) return;
		const { scrollLeft, scrollWidth, clientWidth } = newsScrollContainer.value;
    ...
    >
        ...
        >
            ...
            <div class="absolute top-1/2 left-[max(-7rem,calc(50%-52rem))] -z-10 -translate-y-1/2 transform-gpu blur-2xl" aria-hidden="true">
                <div class="aspect-577/310 w-144.25 bg-linear-to-r from-[#60a5fa] to-[#a78bfa] opacity-30" style="clip-path: polygon(74.8% 41.9%, 97.2% 73.2%, 100% 34.9%, 92.5% 0.4%, 87.5% 0%, 75% 28.6%, 58.5% 54.6%, 50.1% 56.8%, 46.9% 44%, 48.3% 17.4%, 24.7% 53.9%, 0% 27.9%, 11.9% 74.2%, 24.9% 54.1%, 68.6% 100%, 74.8% 41.9%)"></div>
            </div>
            ...
            <div class="flex flex-wrap items-center gap-x-4 gap-y-2 max-w-full">
                <p class="text-sm leading-6 text-white font-sans">
                    <strong class="font-semibold text-white">{{ pinned_announcement.category || 'Penting' }}</strong>
                    <svg viewBox="0 0 2 2" class="mx-2 inline w-1.5 h-1.5 fill-current text-white/80" aria-hidden="true">
            ...
            <!-- Close Button -->
            <div class="absolute right-3 top-1/2 -translate-y-1/2 sm:relative sm:top-auto sm:-translate-y-0 sm:right-auto flex sm:flex-1 sm:justify-end">
                <button 
                    type="button" 
                    @click="dismissBanner" 
        ...
        <section v-if="settings?.show_features !== '0'" id="news" class="bg-white py-20 lg:py-28 overflow-hidden">
            <div class="mx-auto max-w-[1216px] px-4">
                <!-- Header with navigation arrows on the right -->
                <div class="mb-10 flex flex-col justify-between md:mb-14 md:flex-row md:items-end">
                    <div class="max-w-2xl">
                        <h2 class="text-4xl lg:text-5xl font-bold text-[#111827] mb-4 tracking-tight">Info Terbaru FMIKOM</h2>
                        <p class="text-[17px] text-[#4b5563] leading-relaxed">
            ...
            <Deferred data="latest_posts">
                <template #fallback>
                    <div class="w-full">
                        <div class="flex gap-6 overflow-x-auto hide-scrollbar px-4 md:pl-[max(1rem,calc((100vw-1216px)/2+1rem))] md:pr-[max(1rem,calc((100vw-1216px)/2+1rem))]">
                            <div v-for="i in 3" :key="i" class="shrink-0 w-[85vw] sm:w-[420px] md:w-[452px] flex flex-col justify-between animate-pulse">
                                <div>
                                    <div class="relative w-full aspect-16/10 rounded-xl bg-slate-100 border border-slate-200/60 shadow-xs mb-5"></div>
                                    <div class="h-4 bg-slate-100 rounded w-1/3 mb-3"></div>
                                    <div class="h-6 bg-slate-100 rounded w-3/4 mb-3"></div>
                                    <div class="h-4 bg-slate-100 rounded w-5/6 mb-2"></div>
                                    <div class="h-4 bg-slate-100 rounded w-2/3 mb-6"></div>
                                </div>
                                <div class="h-4 bg-slate-100 rounded w-1/4 mt-2"></div>
                            </div>
                        </div>
                    </div>
                </template>
                <template #default>
                    <div v-if="latest_posts.length === 0" class="mx-auto max-w-[1216px] px-4 py-12 text-center text-gray-500 font-medium">
                        Belum ada postingan yang dipublikasikan.
                    </div>
                    <div v-else class="w-full">
                        <!-- Navigation Buttons (rendered dynamically if we have posts) -->
                        <div class="mx-auto max-w-[1216px] px-4 flex justify-end gap-2.5 -mt-20 mb-10 relative z-20">
                            <button
                        ...
                        >
                            <div 
                                ...
                                @click="$inertia.visit('/berita/' + post.slug)"
                            >
                                <div>
                                    <div class="relative w-full aspect-16/10 rounded-xl overflow-hidden bg-slate-50 border border-slate-100 shadow-xs mb-5">
                                        <div class="w-full h-full origin-bottom transition duration-300 group-hover:scale-102">
                                            <img v-if="post.thumbnail" :src="optimizeImageUrl(post.thumbnail, 600)" :srcset="getUnsplashSrcset(post.thumbnail)" sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 452px" :alt="post.title" class="w-full h-full object-cover object-center" width="452" height="283" loading="lazy" decoding="async" />
                                            <div v-else class="w-full h-full flex items-center justify-center text-slate-300">
                                                <FileText class="w-16 h-16" />
                                            </div>
                                        </div>
                                    </div>
                                    <span class="text-sm font-medium text-slate-600 mb-3 flex items-center gap-1.5">
                                        {{ toTitleCase(post.user?.name || 'Admin') }}
                                        <span class="text-slate-300">&bull;</span>
                                        {{ formatDate(post.published_at || post.created_at) }}
                                    </span>
                                    <h4 class="text-xl md:text-2xl font-semibold text-slate-900 leading-tight mb-3 group-hover:text-blue-600 transition-colors line-clamp-3 tracking-tight">
                                        {{ post.title }}
                                    </h4>
                                    <p v-if="getPostExcerpt(post, 120)" class="text-[15px] text-slate-600 leading-relaxed mb-6 line-clamp-2">
                                        {{ getPostExcerpt(post, 120) }}
                                    </p>
                                </div>
                                <div class="flex items-center text-sm font-semibold text-blue-600 mt-2">
                                    Baca selengkapnya
        ...
        <!-- PARTNERS SECTION -->
        <section v-if="settings?.show_partners !== '0' && settings?.partners?.length > 0" class="border-t border-gray-100 bg-gray-50/30 pt-12 pb-24 overflow-hidden" v-once>
            <div class="mx-auto max-w-7xl px-4 xl:px-0 mb-8">
                <p class="text-center text-xs font-bold tracking-widest text-slate-600 uppercase">Telah Bekerja Sama Dengan</p>
            </div>
                ...
                <div class="flex w-max animate-marquee">
                    ...
                    <div class="flex shrink-0 items-center">
                        <div v-for="(partner, i) in settings.partners" :key="'p1-' + i" class="group flex h-20 w-40 sm:h-24 sm:w-56 items-center justify-center border-r border-gray-200 transition-all hover:bg-gray-100/50">
                            <img :src="typeof partner === 'object' ? partner.logo : partner" :alt="typeof partner === 'object' && partner.name ? partner.name : 'Partner Logo'" class="h-10 w-28 sm:h-12 sm:w-40 object-contain grayscale opacity-60 transition-all duration-300 group-hover:scale-110 group-hover:grayscale-0 group-hover:opacity-100" width="160" height="48" loading="lazy" decoding="async">
                        </div>
                    </div>
        ...
        <div class="progressive-blur pointer-events-none fixed bottom-0 left-0 right-0 z-40 h-[140px] select-none">
            <div></div>
            <div></div>
            <div></div>
            <div></div>
...
.progressive-blur div:nth-child(1) {
    ...
    mask-image: linear-gradient(to top, rgba(0,0,0,1) 0%, rgba(0,0,0,0) 20%);
