        ...
        <div class="grid gap-5">
            ...
            >
                ...
                <CardContent class="px-0 py-0">
                    <template v-if="hasPendingRegistrations">
                        <div class="overflow-x-auto">
                            <div class="min-w-[760px]">
                                <div class="grid grid-cols-[minmax(0,1fr)_minmax(190px,0.7fr)_140px_88px] items-center gap-4 border-b border-zinc-200 bg-zinc-50 px-5 py-2.5 text-[11px] font-bold tracking-[0.16em] text-slate-500 uppercase">
                                    <span>Mahasiswa</span>
                                ...
                                <div
                                    v-for="item in pendingRegistrations"
                                    :key="item.id"
                                    class="grid grid-cols-[minmax(0,1fr)_minmax(190px,0.7fr)_140px_88px] items-center gap-4 border-b border-zinc-100 px-5 py-4 transition-colors last:border-b-0 hover:bg-zinc-50"
                                >
                                    <div class="min-w-0">
                                    ...
                                    <div class="flex items-center gap-2 text-sm text-zinc-600">
                                        <CalendarClock class="size-4 shrink-0 text-zinc-400" />
                                        <span>{{ item.date_range || '-' }}</span>
                                    </div>
                                    <div>
                                        <Badge class="rounded-full border border-blue-200 bg-blue-50 px-2.5 py-0.5 text-[10px] font-bold text-blue-700 shadow-none">
                                            {{ registrationStatusLabel(item.status) }}
                                        </Badge>
                                    </div>
                                    <div class="flex justify-end">
                                        <Button as-child class="h-9 rounded-xl bg-gradient-to-r from-blue-600 to-blue-500 px-3.5 text-sm font-bold text-white shadow-lg shadow-blue-500/20 transition-all hover:shadow-blue-500/30 active:scale-[0.98] dark:from-[#214FAF] dark:to-[#0F6FBE] dark:shadow-[0_14px_34px_-18px_rgba(8,15,30,0.84)] dark:hover:shadow-[0_18px_38px_-18px_rgba(8,15,30,0.92)]">
                                            <Link :href="wimsRoutes.admin.registrations.index().url">
                                                Detail
                                            </Link>
                                        </Button>
                                    </div>
