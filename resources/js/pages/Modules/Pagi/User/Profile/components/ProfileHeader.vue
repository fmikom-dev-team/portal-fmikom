...
<template>
	...
	<div class="hidden lg:flex lg:flex-row justify-between items-start gap-10 mb-12 transition-all duration-300">
		...
		<div class="flex-1 w-full">
			...
			<div v-else class="flex flex-col items-start gap-5 w-full">
				...
				<div class="flex items-center gap-2.5 w-full sm:w-auto pt-1">
					...
					<button 
						@click="isFollowing ? emit('request-unfollow') : emit('toggle-follow')" 
						class="h-[38px] px-5 rounded-full border transition-all duration-200 font-bold text-xs uppercase tracking-wider cursor-pointer shadow-2xs active:scale-98 flex items-center justify-center gap-1.5 group/followbtn"
						:class="isFollowing 
							? 'bg-indigo-600 border-transparent text-white hover:bg-red-600 hover:border-red-600' 
					...
					>
						<span v-if="isFollowing" class="group-hover/followbtn:hidden">Mengikuti</span>
						<span v-if="isFollowing" class="hidden group-hover/followbtn:inline">Unfollow</span>
		...
		<div 
			...
			:role="isOwnProfile ? 'button' : undefined"
			:aria-label="isOwnProfile ? 'Ubah Banner Profil' : undefined"
		>
			<!-- Tooltip Bubble -->
		...
		<div class="mt-8 flex flex-col items-start w-full px-1">
			...
			<div class="flex items-center gap-3 mt-4 w-full">
				...
				<div v-if="!isOwnProfile" class="flex-1">
					<button 
						@click="isFollowing ? emit('request-unfollow') : emit('toggle-follow')" 
						class="w-full h-11 px-5 rounded-full font-black text-xs uppercase tracking-wider transition-all duration-200 cursor-pointer shadow-2xs active:scale-97 flex items-center justify-center gap-1.5 group/followbtnm"
						:class="isFollowing 
							? 'bg-indigo-600 text-white hover:bg-red-600 border-transparent' 
					...
					>
						<span v-if="isFollowing" class="group-hover/followbtnm:hidden">Mengikuti</span>
						<span v-if="isFollowing" class="hidden group-hover/followbtnm:inline">Unfollow</span>
						<span v-if="!isFollowing">Follow</span>
					</button>
				</div>
