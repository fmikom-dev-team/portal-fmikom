...
const toggleProjectMenu = (id: number) => {
	if (activeProjectMenu.value === id) {
		activeProjectMenu.value = null;
	} else {
		activeProjectMenu.value = id;
	}
};
			...
			>
				...
				<div v-if="isOwnProfile" class="absolute top-3 right-3 opacity-0 group-hover:opacity-100 transition-opacity z-20 flex items-center bg-white/95 dark:bg-slate-900/95 backdrop-blur-xs border border-slate-200 dark:border-slate-800 rounded-full p-1 shadow-md gap-0.5 select-none">
					...
					<button 
						...
						class="w-7 h-7 rounded-full flex items-center justify-center text-slate-600 dark:text-slate-355 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-pointer transition-colors border-none bg-transparent"
						title="Options"
					>
					...
					<!-- Drag Handle Icon -->
					<div 
						class="w-7 h-7 rounded-full flex items-center justify-center text-slate-600 dark:text-slate-355 hover:bg-slate-100 dark:hover:bg-slate-800 cursor-grab active:cursor-grabbing transition-colors"
						title="Drag to reorder"
					>
						<svg class="w-4 h-4" fill="none" stroke="currentColor" stroke-width="2.5" viewBox="0 0 24 24">
							<path stroke-linecap="round" stroke-linejoin="round" d="M8 9h.01M12 9h.01M16 9h.01M8 15h.01M12 15h.01M16 15h.01" />
						</svg>
					</div>
					...
					<div v-if="activeProjectMenu === p.id" class="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl rounded-2xl py-1.5 z-30 overflow-hidden">
						<Link 
							:href="'/pagi/editor?id=' + p.id" 
							class="w-full flex items-center gap-2.5 px-4 py-2.5 text-xs font-bold text-slate-700 dark:text-slate-200 hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
						>
				...
				<!-- Hover Overlay with interactive Like -->
				<div class="absolute inset-0 bg-gradient-to-t from-black/90 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-all duration-300 flex flex-col justify-end p-4 z-10 select-none">
					...
					<div class="flex items-center gap-3 text-[10px] text-white/90 font-medium">
						...
						>
							<Heart class="w-3.5 h-3.5 transition-all duration-200" :class="getLiked(p) ? 'fill-red-400 stroke-red-400 scale-110' : 'fill-white/20 stroke-white group-hover/like:fill-red-400 group-hover/like:stroke-red-400'" />
