...
import PagiProfileCompletenessBanner from "./ui/PagiProfileCompletenessBanner.vue";
...
<template>
    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 selection:bg-slate-200 dark:selection:bg-slate-800 overflow-x-clip">
        ...
        <!-- HERO BANNER -->
        <div class="relative overflow-hidden bg-linear-to-br from-[#030712] via-[#0b0f19] to-[#1e1b4b] border-b border-slate-900 py-8 sm:py-10 md:py-12 px-4 text-center select-none">
            ...
            <div class="relative max-w-2xl mx-auto space-y-2.5">
                <h1 class="text-2xl sm:text-4xl font-black text-white tracking-tight leading-tight drop-shadow-sm">
                    Build Your Academic & <br class="hidden sm:inline" />
                    Creative Network
                </h1>
                <p class="text-[11px] sm:text-xs leading-relaxed text-slate-300 max-w-lg mx-auto font-medium">
        ...
        <div class="border-b border-slate-200/80 dark:border-zinc-800/80 bg-white dark:bg-zinc-900 py-2.5 sm:py-3.5 select-none sticky top-14 z-40">
            <div class="mx-auto max-w-[1400px] px-3 sm:px-4 flex items-center gap-2 sm:gap-3">
                ...
                <button @click="showFilters = !showFilters" aria-label="Filter kategori" class="inline-flex items-center gap-1.5 rounded-full border border-slate-200/85 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800 bg-white dark:bg-zinc-950 px-2.5 sm:px-5 py-2 text-xs font-bold text-slate-800 dark:text-zinc-200 shadow-xs transition-all shrink-0">
                    <SlidersHorizontal class="h-3.5 w-3.5 text-slate-600 dark:text-zinc-300" />
                    <span class="hidden sm:inline">Filter</span>
                </button>

                <!-- Center: Search Input -->
                <div class="flex-1 relative rounded-full border border-slate-200/85 dark:border-zinc-700 bg-slate-50/50 dark:bg-zinc-900/40 hover:border-slate-300 dark:hover:border-zinc-600 flex items-center pl-3.5 pr-1.5 py-1 transition-all min-w-0">
                    <Search class="h-3.5 w-3.5 sm:h-4 sm:w-4 text-slate-500 dark:text-zinc-300 shrink-0" />
                    <input
                        type="text"
                        v-model="searchQuery"
                        ...
                        class="flex-1 bg-transparent border-none outline-none text-xs font-semibold text-slate-800 dark:text-zinc-100 placeholder-slate-400 dark:placeholder-zinc-500 pl-2 py-1.5 w-0 min-w-0"
                    />

                    <!-- REALTIME SEARCH SUGGESTIONS DROPDOWN -->
                    <div v-show="showSuggestions && searchSuggestions.length > 0" class="absolute left-0 right-0 top-full mt-2 bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-700 rounded-2xl shadow-xl z-50 overflow-hidden p-1.5">
                        <div class="px-3 py-1.5 text-[9px] font-black uppercase tracking-widest text-slate-400 dark:text-zinc-400 border-b border-slate-100 dark:border-zinc-800/80 mb-1">
                        ...
                        >
                            ...
                            <span v-if="sug.type === 'title'" class="ml-auto text-[9px] font-bold uppercase tracking-wide text-indigo-600 dark:text-indigo-400 bg-indigo-50 dark:bg-indigo-950/40 px-2 py-0.5 rounded-md">Prodi</span>
                            <span v-else-if="sug.type === 'author'" class="ml-auto text-[9px] font-bold uppercase tracking-wide text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-950/40 px-2 py-0.5 rounded-md">Nama</span>
                        </button>
                    </div>

                    <!-- Search Internal Navigation Tabs (desktop only) -->
                    <div class="hidden md:flex items-center gap-0.5 bg-white dark:bg-zinc-900 rounded-full p-0.5 border border-slate-100 dark:border-zinc-700 shadow-xs shrink-0">
                ...
                <div class="relative shrink-0">
                    <button @click="showSortDropdown = !showSortDropdown" aria-label="Urutkan kreator" class="inline-flex items-center gap-1.5 rounded-full border border-slate-200/85 dark:border-zinc-700 hover:bg-slate-50 dark:hover:bg-zinc-800 bg-white dark:bg-zinc-950 px-2.5 sm:px-5 py-2 text-xs font-bold text-slate-800 dark:text-zinc-200 shadow-xs transition-all">
                    ...
                    <div v-show="showSortDropdown" class="absolute right-0 top-full mt-2 w-48 rounded-2xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 shadow-lg p-1.5 z-50">
        ...
        <!-- GRID -->
        <main class="mx-auto max-w-[1400px] px-4 py-8 pb-28 md:pb-8">
            <PagiProfileCompletenessBanner :status="profileVisibilityStatus" />

            <Deferred data="peopleYouMayKnow">
                <template #fallback>
                    <!-- SKELETON LOADER -->
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5 select-none">
                        <div v-for="n in 8" :key="n" class="rounded-xl border border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-950 overflow-hidden p-5 flex flex-col gap-4">
                            <div class="relative grid grid-cols-3 gap-0.5 h-36 bg-slate-100 dark:bg-zinc-850 overflow-hidden rounded-lg">
                                ...
                                <div class="h-36 bg-slate-200/50 dark:bg-zinc-900/40"></div>
                                <Skeleton class="h-20 w-20 rounded-full border-4 border-white dark:border-zinc-800 absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 shrink-0 z-10" />
                            </div>
                ...
                <template #default>
                    <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-5">
                        ...
                        >
                            <div class="group rounded-xl border border-slate-200 dark:border-zinc-700/80 bg-white dark:bg-zinc-900 overflow-hidden hover:shadow-xl hover:border-slate-300 dark:hover:border-zinc-600 hover:-translate-y-1 transition-all duration-200 cursor-pointer"
                            ...
                            <div v-if="p.imgs && p.imgs.length > 0" class="relative grid grid-cols-3 gap-0.5 bg-slate-100 dark:bg-zinc-850 h-36">
                                ...
                                <div class="absolute left-1/2 -translate-x-1/2 bottom-0 translate-y-1/2 z-10">
                                    <OptimizedImage :src="p.avatar"
                                        className="h-20 w-20 rounded-full border-4 border-white dark:border-zinc-800 object-cover shadow-lg"
                                        alt="Author avatar" />
                                </div>
                            </div>
                            ...
                            <div class="px-5 pt-12 pb-5">
                                ...
                                <div class="text-center mb-3">
                                    <div class="flex flex-col items-center justify-center gap-0.5 mb-1">
                                        <div class="flex items-center gap-1.5">
                                            <h3 class="text-sm font-bold text-slate-800 dark:text-white leading-tight">{{ p.name }}</h3>
                                            <img src="/premium.svg" class="w-3.5 h-3.5 shrink-0 select-none" title="Akun Terverifikasi" alt="Verified Badge" />
                                        </div>
                                        <span v-if="p.pagi_username" class="text-[10px] font-bold text-slate-500 dark:text-zinc-400">@{{ p.pagi_username }}</span>
                                    </div>
                                    <div class="flex items-center justify-center gap-1 text-xs text-slate-500 dark:text-zinc-400">
                                ...
                                <div class="flex flex-wrap justify-center gap-1.5 mb-4 min-h-[26px]">
                                    <span v-for="skill in p.skills.slice(0, 2)" :key="skill"
                                        class="rounded-full bg-slate-100 dark:bg-zinc-800/80 text-slate-700 dark:text-zinc-200 border border-slate-200/50 dark:border-zinc-700/60 px-2.5 py-1 text-[11px] font-bold tracking-tight">
                                        {{ skill }}
                                    </span>
                                </div>
                                ...
                                <div class="grid grid-cols-3 gap-2 mb-4">
                                    <div class="text-center">
                                        ...
                                        <p class="text-[10px] text-slate-400 dark:text-zinc-400">Likes</p>
                                    </div>
                                    <div class="text-center border-x border-slate-200/80 dark:border-zinc-700/80">
                                        <p class="text-sm font-black text-slate-800 dark:text-zinc-100">{{ p.fol }}</p>
                                        <p class="text-[10px] text-slate-400 dark:text-zinc-400">Followers</p>
                                    </div>
                                    <div class="text-center">
                                        <p class="text-sm font-black text-slate-800 dark:text-zinc-100">{{ p.proj }}</p>
                                        <p class="text-[10px] text-slate-400 dark:text-zinc-400">Projects</p>
                                    </div>
                                </div>

                                <!-- Message Button -->
                                <button v-if="p.id !== user.id && p.is_message_enabled !== false" @click.stop="startChat(p.id)" class="w-full rounded-lg border border-slate-200 dark:border-zinc-700 py-2 text-xs font-bold text-slate-700 dark:text-zinc-100 dark:hover:text-white hover:bg-slate-50 dark:hover:bg-zinc-800/80 transition-colors cursor-pointer shadow-3xs">
