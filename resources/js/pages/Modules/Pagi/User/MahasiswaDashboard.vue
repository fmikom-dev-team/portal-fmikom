...
const featuredProjects = computed(() => visibleProjects.value.slice(0, 4));
const regularProjects = computed(() => visibleProjects.value.slice(4));
...
<template>
    ...
    <div class="min-h-screen bg-slate-50 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 selection:bg-slate-200 dark:selection:bg-slate-800">
        ...
        <div v-show="showFilters" class="sticky top-[100px] sm:top-[124px] z-30 border-b border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-950/80 backdrop-blur-md">
            <div class="mx-auto max-w-[1400px] px-3 sm:px-4">
                <div class="flex items-center gap-1.5 sm:gap-2 py-3 sm:py-4 overflow-x-auto" style="scrollbar-width:none;">
                    ...
                    <button class="shrink-0 p-2 rounded-xl bg-slate-100 dark:bg-slate-900 text-slate-500 dark:text-slate-450 hover:bg-slate-200 dark:hover:bg-slate-800 transition-colors">
        ...
        <!-- MAIN -->
        <main class="mx-auto max-w-[1400px] px-4 pt-10 pb-24 md:pb-16 transition-all duration-300">            <!-- Featured Grid -->
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2 mb-8">
                <div v-for="p in featuredProjects" :key="p.id" class="group" @click.self="closeMenu">
                    <div @click="openProjectModal(p)" class="relative rounded-md overflow-hidden aspect-4/3 bg-slate-100 dark:bg-slate-900 border border-slate-200/50 dark:border-slate-800 mb-2 cursor-pointer">
                        ...
                        <div class="absolute top-3 right-3 z-20 opacity-0 group-hover:opacity-100 transition-opacity">
                            <div class="relative">
                                <button @click.stop="toggleMenu(p.id, $event)" class="w-7 h-7 rounded-full bg-black/60 backdrop-blur-md hover:bg-black/80 flex items-center justify-center transition-all shadow-md cursor-pointer border-none" title="Opsi">
                            ...
                            <div class="w-full p-3 opacity-0 group-hover:opacity-100 translate-y-1 group-hover:translate-y-0 transition-all duration-300">
                                <div class="flex items-center justify-between">
                                    <div class="flex items-center gap-3 text-white text-xs">
                                        <button @click.stop="handleLikeProject(p)" class="flex items-center gap-1 focus:outline-none transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer text-white" :class="{ 'text-red-500 hover:text-red-400': p.liked, 'hover:text-red-500': !p.liked }">
                    ...
                    <div class="flex items-center gap-2">
                        <div v-if="getAcceptedCollaborators(p).length > 0" class="flex -space-x-2 shrink-0">
                            ...
                            <template v-for="(collab, idx) in getAcceptedCollaborators(p).slice(0, 2)" :key="collab.id">
                                <Link :href="collab.pagi_username ? '/pagi/' + collab.pagi_username : '/pagi/profile/' + collab.id" class="shrink-0" :style="{ zIndex: 9 - idx }">
                        ...
                        <div class="min-w-0 flex-1">
                            ...
                            <div class="flex items-center gap-1 mt-0.5 min-w-0">
                                <Link v-if="p.user" :href="p.user.pagi_username ? '/pagi/' + p.user.pagi_username : '/pagi/profile/' + p.user.id" class="text-[10px] text-slate-400 dark:text-slate-500 hover:text-indigo-500 dark:hover:text-indigo-400 hover:underline transition-colors truncate block">{{ p.author }}</Link>
                                <p v-else class="text-[10px] text-slate-400 dark:text-slate-500 truncate">{{ p.author }}</p>
                        ...
                        <div class="flex items-center gap-1.5 text-[10px] text-slate-400 dark:text-slate-500 shrink-0 select-none">
                            <button @click.stop="handleLikeProject(p)" class="flex items-center gap-0.5 transition-all duration-200 focus:outline-none hover:scale-110 active:scale-90 cursor-pointer" :class="p.liked ? 'text-red-500 dark:text-red-400 font-bold' : 'text-slate-400 dark:text-slate-500 hover:text-red-500 dark:hover:text-red-455'">
            ...
            <div class="grid grid-cols-2 lg:grid-cols-4 gap-2">
                <div v-for="p in regularProjects" :key="p.id" class="group" @click.self="closeMenu">
        ...
        <div class="fixed top-6 right-6 z-[10010] flex flex-col gap-3.5 max-w-xs pointer-events-none">
            ...
            >
                ...
                >
                    ...
                    <button @click="toasts = toasts.filter(t => t.id !== toast.id)" class="text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 shrink-0 cursor-pointer border-none bg-transparent p-0 flex items-center justify-center">
                        <X class="w-3.5 h-3.5" />
                    </button>
                </div>
            </TransitionGroup>
