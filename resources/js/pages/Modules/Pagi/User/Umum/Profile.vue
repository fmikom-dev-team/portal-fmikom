...
const WorkTab = defineAsyncComponent(() => import("../Profile/WorkTab.vue"));
const GalleryTab = defineAsyncComponent(
	() => import("../Profile/GalleryTab.vue"),
);
const SertifikatTab = defineAsyncComponent(
	() => import("../Profile/SertifikatTab.vue"),
);
const AboutTab = defineAsyncComponent(() => import("../Profile/AboutTab.vue"));

const PagiShareModal = defineAsyncComponent(
	() => import("../ui/PagiShareModal.vue"),
);
const AddWorkModal = defineAsyncComponent(
	() => import("../Profile/components/AddWorkModal.vue"),
);
const CropImageModal = defineAsyncComponent(
	() => import("../Profile/components/CropImageModal.vue"),
);
const EditAvatarModal = defineAsyncComponent(
	() => import("../Profile/components/EditAvatarModal.vue"),
);
const EditBannerModal = defineAsyncComponent(
	() => import("../Profile/components/EditBannerModal.vue"),
);
const EditBioModal = defineAsyncComponent(
	() => import("../Profile/components/EditBioModal.vue"),
);
const EditDetailsModal = defineAsyncComponent(
	() => import("../Profile/components/EditDetailsModal.vue"),
);
const EditLocationModal = defineAsyncComponent(
	() => import("../Profile/components/EditLocationModal.vue"),
);
const EditSocialsModal = defineAsyncComponent(
	() => import("../Profile/components/EditSocialsModal.vue"),
);
const EditUsernameModal = defineAsyncComponent(
	() => import("../Profile/components/EditUsernameModal.vue"),
);

import ProfileHeader from "../Profile/components/ProfileHeader.vue";
import ProfileTabs from "../Profile/components/ProfileTabs.vue";

const RelationsModal = defineAsyncComponent(
	() => import("../Profile/components/RelationsModal.vue"),
);
const WarningModal = defineAsyncComponent(
	() => import("../Profile/components/WarningModal.vue"),
);

// Composables
import { useProfileForm } from "../Profile/composables/useProfileForm";
import { useProfileProjects } from "../Profile/composables/useProfileProjects";
...
const toggleFollow = async () => {
	...
	try {
		...
		if (page.props.auth?.user) {
			...
			if (data.following) {
				list.push(user.value.id);
			} else {
				list = list.filter((id: number) => id !== user.value.id);
			}
...
<template>
	...
	<div class="min-h-screen bg-slate-100 dark:bg-slate-950 font-sans text-slate-900 dark:text-slate-100 selection:bg-slate-900 selection:text-white dark:selection:bg-white dark:selection:text-slate-900" style="font-family:'Inter',system-ui,sans-serif;">
		...
		<UmumNavbar :roleName="displayRoleName" />
