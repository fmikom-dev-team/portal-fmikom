...
<template>
    <div class="space-y-5 animate-fade-in max-w-[800px]">
        ...
        <div class="border-b border-gray-200 pb-5">
            <h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Features</h1>
            <p class="text-[14px] text-gray-500 mt-1">Manage global platform features.</p>
        </div>

        <!-- Loading -->
        <div v-if="isLoading" class="space-y-4">
            <div v-for="i in 6" :key="i" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-3">
                ...
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1 min-w-0">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">Hosted UI</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Users sign in to your app using a customizable hosted authentication UI.</p>
                            </div>
                            <div :class="['flex items-center gap-1 text-[13px] font-medium shrink-0', features.hosted_ui.enabled ? 'text-emerald-600' : 'text-gray-400']">
                                <Check v-if="features.hosted_ui.enabled" class="w-3.5 h-3.5" />
                                {{ features.hosted_ui.enabled ? 'Enabled' : 'Disabled' }}
                            </div>
                        </div>
                        <div class="flex flex-wrap items-center gap-2 mt-3">
                            <button @click="openModal('hosted_ui')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                            <div v-if="features.hosted_ui.authkit_url || features.hosted_ui.redirect_uri" class="flex items-center gap-1.5 text-[12px] text-gray-500 border border-gray-200 rounded-md px-2.5 py-1.5 bg-gray-50">
                                <span class="truncate max-w-[200px]">{{ features.hosted_ui.authkit_url || features.hosted_ui.redirect_uri }}</span>
                                <span class="text-[10px] font-semibold text-gray-600 bg-gray-200 px-1.5 py-0.5 rounded shrink-0">Staging</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">Sign-up</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Allow users to sign up for your app and create their own user accounts.</p>
                            </div>
                            <div :class="['flex items-center gap-1 text-[13px] font-medium shrink-0', features.sign_up.enabled ? 'text-emerald-600' : 'text-gray-400']">
                                <Check v-if="features.sign_up.enabled" class="w-3.5 h-3.5" />
                                {{ features.sign_up.enabled ? 'Enabled' : 'Disabled' }}
                            </div>
                        </div>
                        <div class="flex items-center gap-2 mt-3">
                            <button v-if="features.sign_up.enabled" @click="openModal('sign_up')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                            <button v-else @click="enableFeature('sign_up')" :disabled="saving === 'sign_up'" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-1.5">
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">Invitations</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Configure the expiration period for user invitations to join an organization.</p>
                            </div>
                        </div>
                        <!-- Detail row -->
                        <div class="flex items-center gap-6 mt-3 mb-3">
                            <div class="text-[13px] text-gray-500">Default invitation expiry</div>
                            <div class="text-[13px] text-gray-900 font-medium">{{ features.invitations.expiry_days }} days</div>
                        </div>
                        <button @click="openModal('invitations')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">Multi-factor auth</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Require non-SSO users to set up multi-factor authentication to sign in.</p>
                            </div>
                            <div :class="['flex items-center gap-1 text-[13px] font-medium shrink-0', mfaEnabled ? 'text-emerald-600' : 'text-gray-400']">
                                <Check v-if="mfaEnabled" class="w-3.5 h-3.5" />
                                <span v-else class="w-3.5 h-3.5 border-2 border-gray-300 rounded-full inline-block"></span>
                                {{ mfaStatusLabel }}
                            </div>
                        </div>
                        <div class="flex items-center gap-2 mt-3">
                            <button v-if="mfaEnabled" @click="openModal('mfa')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                            <button v-else @click="openModal('mfa')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Enable</button>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">Localization</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Automatically display AuthKit's hosted UI and authentication emails in the user's browser language.</p>
                            </div>
                            <div :class="['flex items-center gap-1 text-[13px] font-medium shrink-0', features.localization.enabled ? 'text-emerald-600' : 'text-gray-400']">
                        ...
                        <div class="flex items-center gap-6 mt-3 mb-3" v-if="features.localization.enabled">
                            <div class="text-[13px] text-gray-500">Fallback language</div>
                            <div class="text-[13px] text-gray-900 font-medium">{{ selectedLangLabel }}</div>
                        </div>
                        <div class="flex items-center gap-2 mt-3">
                            <button v-if="features.localization.enabled" @click="openModal('localization')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                            <button v-else @click="enableFeature('localization')" :disabled="saving === 'localization'" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-1.5">
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                <div class="flex items-start gap-4">
                    ...
                    <div class="flex-1">
                        <div class="flex items-start justify-between gap-4">
                            <div>
                                <h3 class="text-[14px] font-semibold text-gray-900">User impersonation</h3>
                                <p class="text-[13px] text-gray-500 mt-0.5">Allow team members in the dashboard to sign in as any user of your app.</p>
                            </div>
                            <div :class="['flex items-center gap-1 text-[13px] font-medium shrink-0', features.user_impersonation.enabled ? 'text-emerald-600' : 'text-gray-400']">
                        ...
                        <div class="flex items-center gap-2 mt-3">
                            <button v-if="features.user_impersonation.enabled" @click="openModal('user_impersonation')" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm">Manage</button>
                            <button v-else @click="enableFeature('user_impersonation')" :disabled="saving === 'user_impersonation'" class="px-3 py-1.5 bg-white border border-gray-200 rounded-md text-[13px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm flex items-center gap-1.5">
            ...
            <div class="space-y-5 text-[13px]">
                ...
                <div>
                    ...
                    <div class="mt-3 bg-gray-50 border border-gray-200 rounded-lg p-3 text-gray-600 text-[12.5px] flex gap-2">
                        <Shield class="w-4 h-4 mt-0.5 text-gray-400 shrink-0" />
        ...
        <AppModal :show="activeModal === 'invitations'" title="Invitations" description="Configure the expiration period for user invitations to join an organization (maximum of 30 days)." size="md" @close="activeModal = null">
            <div class="space-y-4 text-[13px]">
                <div>
                    <label class="block font-medium text-gray-900 mb-2">Default invitation expiry</label>
                    ...
                    <p class="text-gray-500 text-[12px] mt-1.5">Maximum is 30 days. After expiry, invitations must be resent.</p>
                </div>
            </div>
            <template #footer>
                <button @click="activeModal = null" class="px-4 py-2 bg-white border border-gray-200 rounded-lg text-[13px] font-medium text-gray-700 hover:bg-gray-50">Cancel</button>
        ...
        <AppModal :show="activeModal === 'localization'" title="Localization" size="md" @close="activeModal = null; langDropdownOpen = false">
            <div class="space-y-5 text-[13px]">
                ...
                <p class="text-gray-600 text-[13px] -mt-2">Automatically display AuthKit's hosted UI and authentication emails sent by WorkOS in the user's browser language.</p>

                <hr class="border-gray-200" />

                <!-- Fallback language searchable dropdown -->
                <div>
                    <label class="block font-medium text-gray-900 mb-2">Fallback language</label>
                    <div class="relative">
                        <button
