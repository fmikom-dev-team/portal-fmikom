...
<template>
    <div class="space-y-6 animate-fade-in max-w-[800px]">
        ...
        <div class="flex flex-col sm:flex-row sm:items-start justify-between gap-4 border-b border-gray-200 pb-5">
            <div>
                <h1 class="text-2xl font-semibold text-gray-900 tracking-tight">Multi-factor Authentication</h1>
                <p class="text-[14px] text-gray-500 mt-1">Secure your accounts by requiring a second form of authentication.</p>
            </div>
        </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
            <div class="p-6">
                <div class="flex items-start gap-4">
                    ...
                    <div>
                        <h3 class="text-base font-semibold text-gray-900 mb-1">Authenticator App (TOTP)</h3>
                        <p class="text-[13px] text-gray-500 mb-4">Use an app like 1Password, Authy, or Google Authenticator to generate one-time codes.</p>
                        ...
                        <div v-else-if="isActive && !isSetupOpen" class="bg-emerald-50 border border-emerald-200 rounded-lg p-4 mb-4 flex items-center justify-between">
                            ...
                            <button 
                                @click="startMfaSetup" 
                                class="px-3 py-1.5 bg-white border border-gray-300 rounded-md text-[12px] font-medium text-gray-700 hover:bg-gray-50 transition-colors shadow-sm"
                            >
                                Buat Baru (Reset)
                            </button>
                        </div>
                        ...
                        <!-- Setup Flow -->
                        <div v-if="isSetupOpen" class="mt-6 border border-gray-200 rounded-lg p-5 bg-gray-50/50">
                            
                            <div v-if="!isVerified">
                                <h4 class="text-[14px] font-semibold text-gray-900 mb-4">1. Scan this QR Code</h4>
                                <div class="bg-white p-3 rounded-lg border border-gray-200 w-max mb-4 shadow-sm">
                                    <!-- Render the Base64 SVG -->
                                    <img v-if="qrCodeSvg" :src="`data:image/svg+xml;base64,${qrCodeSvg}`" alt="QR Code" class="w-40 h-40" />
                                    <div v-else class="w-40 h-40 bg-gray-100 animate-pulse flex items-center justify-center text-gray-400 text-xs">Loading...</div>
                                </div>
                                <div class="text-[12px] text-gray-500 mb-6 flex items-center gap-2">
                                    <span class="font-medium">Secret:</span> <code class="bg-gray-100 px-1.5 py-0.5 rounded text-gray-800">{{ secret }}</code>
                                </div>

                                <h4 class="text-[14px] font-semibold text-gray-900 mb-3">2. Enter the verification code</h4>
                                <div class="flex items-center gap-3">
                                    <input 
                            ...
                            <div v-else class="animate-fade-in">
                                ...
                                <div class="bg-white border border-gray-200 rounded-lg p-5">
                                    <div class="flex items-start justify-between mb-4">
                                        <div>
                                            <h5 class="text-[14px] font-semibold text-gray-900">Backup Codes</h5>
                                            <p class="text-[13px] text-gray-500">Save these codes in a safe place. You can use them to access your account if you lose your device.</p>
                                        </div>
                                        <button @click="copyCodes" class="text-gray-500 hover:text-gray-700 p-1.5 bg-gray-100 hover:bg-gray-200 rounded-md transition-colors" title="Copy all">
                                            <Copy class="w-4 h-4" />
                                        </button>
                                    </div>
                                    
                                    <div class="grid grid-cols-2 sm:grid-cols-3 gap-3">
                                        <div v-for="code in backupCodes" :key="code" class="bg-gray-50 border border-gray-200 text-gray-700 font-mono text-[13px] px-3 py-2 rounded text-center">
                                            {{ code }}
                                        </div>
                                    </div>
                                </div>
