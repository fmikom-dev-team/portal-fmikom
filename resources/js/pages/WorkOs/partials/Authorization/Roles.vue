    ...
    <div class="p-6 md:p-8">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center gap-3 mb-5">
            ...
            <div class="flex flex-col sm:flex-row items-center gap-2 w-full sm:w-auto">
                <button class="h-9 px-4 rounded-lg text-[13px] border border-gray-200 dark:border-zinc-700 text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900 transition-colors w-full sm:w-auto cursor-pointer">Edit priority</button>
        ...
        <!-- Table -->
        <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl overflow-x-auto shadow-sm dark:shadow-none">
            <table class="w-full text-left whitespace-nowrap">
                ...
                <tbody>
                    <tr v-if="filteredRoles.length === 0">
                        <td colspan="4" class="py-12 text-center text-[13px] text-gray-400 dark:text-zinc-500">No roles found.</td>
                    </tr>
                    <tr v-for="role in filteredRoles" :key="role.id" class="border-b border-gray-100 dark:border-zinc-800 last:border-0 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900/80 transition-colors">
                        <td class="px-5 py-4">
                        ...
                        <td class="px-5 py-4">
                            <button class="text-[13px] text-gray-600 dark:text-zinc-400 hover:text-gray-900 hover:underline transition-colors" @click="openPerms(role)">
                        ...
                        <td class="px-5 py-4 text-right relative">
                            <button class="w-7 h-7 rounded-md flex items-center justify-center text-gray-400 hover:text-gray-700 dark:text-zinc-300 hover:bg-gray-100 dark:hover:bg-zinc-800 dark:bg-zinc-800 transition-colors" @click.stop="toggleMenu(role.id, $event)">
                            ...
                            <div v-if="activeMenu === role.id" class="absolute right-4 top-full mt-1 w-40 bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl shadow-lg z-50 py-1.5 dark:shadow-none">
                                <button class="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900 text-left transition-colors" @click="openEdit(role)">
                                ...
                                <button class="w-full flex items-center gap-2.5 px-3 py-2 text-[13px] text-gray-700 dark:text-zinc-300 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900 text-left transition-colors" @click="openPerms(role)">
        ...
        <Teleport to="body">
                ...
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-md dark:shadow-none">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        <div>
                            ...
                            <input v-model="roleForm.nama" type="text" placeholder="e.g. Dosen Pembimbing" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 placeholder:text-gray-300 transition-all" @input="autoSlug"/>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            <input v-model="roleForm.deskripsi" type="text" placeholder="Short description" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 placeholder:text-gray-300 transition-all"/>
                        </div>
                    </div>
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex justify-end gap-2">
                        ...
                        <button :disabled="loading" class="h-9 px-5 rounded-lg text-[13px] font-semibold bg-gray-900 hover:bg-black text-white shadow-sm transition-colors disabled:opacity-40 dark:shadow-none" @click="submitCreate">{{ loading ? 'Creating…' : 'Create role' }}</button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="modal.editRole" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4" @click.self="modal.editRole = false">
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-md dark:shadow-none">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        <div>
                            <label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Role name</label>
                            <input v-model="roleForm.nama" type="text" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all"/>
                        </div>
                        <div>
                            <label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Slug</label>
                            <div class="flex h-9 border border-gray-300 dark:border-zinc-700 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all">
                        ...
                        <div>
                            ...
                            <input v-model="roleForm.deskripsi" type="text" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all"/>
                        </div>
                    </div>
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex justify-end gap-2">
                        ...
                        <button :disabled="loading" class="h-9 px-4 rounded-lg text-[13px] font-semibold bg-gray-900 hover:bg-black text-white shadow-sm transition-colors disabled:opacity-40 dark:shadow-none" @click="submitEdit">{{ loading ? 'Saving…' : 'Save changes' }}</button>
                    </div>
                </div>
            </div>
            ...
            <div v-if="modal.deleteRole" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4">
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-sm dark:shadow-none">
                    <div class="px-6 py-5">
                        ...
                        <p class="text-[13px] text-gray-500 dark:text-zinc-400">Delete <strong class="text-gray-800 dark:text-zinc-200">{{ deletingRole?.nama }}</strong>? All assignments will be removed. This action cannot be undone.</p>
                    </div>
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex justify-end gap-2">
            ...
            <div v-if="modal.rolePerms" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4" @click.self="modal.rolePerms = false">
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-lg dark:shadow-none">
                    <div class="px-6 py-5 border-b border-gray-100 dark:border-zinc-800">
                        <h2 class="text-[16px] font-semibold text-gray-900 dark:text-zinc-100">Manage Permissions</h2>
                        <p class="text-[12px] text-gray-500 dark:text-zinc-400 mt-0.5">Role: <span class="font-mono font-semibold text-gray-700 dark:text-zinc-300">{{ permRole?.slug }}</span></p>
                    </div>
                    <div class="px-6 py-4 max-h-72 overflow-y-auto">
                        <div v-if="!permissions.length" class="py-6 text-center text-[13px] text-gray-400 dark:text-zinc-500">No permissions defined.</div>
                        <div v-for="(perms, group) in groupedPerms" :key="group" class="mb-4 last:mb-0">
                            <p class="text-[10px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-2">{{ group }}</p>
                            <div class="space-y-0.5">
                                <label v-for="p in perms" :key="p.id" class="flex items-center gap-3 px-3 py-2 rounded-lg hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900 cursor-pointer transition-colors">
                                    <div
                                        :class="['w-4 h-4 rounded border-2 flex items-center justify-center shrink-0 transition-all', selectedPerms.includes(p.id) ? 'bg-gray-900 border-gray-900' : 'bg-white dark:bg-zinc-900 border-gray-300 dark:border-zinc-700']"
                                    ...
                                    <div>
                                        <p class="text-[13px] font-medium text-gray-800 dark:text-zinc-200">{{ p.name }}</p>
                                        <p class="text-[11px] font-mono text-gray-400 dark:text-zinc-500">{{ p.slug }}</p>
                                    </div>
                                </label>
                            </div>
                        </div>
                    </div>
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex items-center justify-between">
                        <span class="text-[12px] text-gray-400 dark:text-zinc-500">{{ selectedPerms.length }} selected</span>
                        <div class="flex gap-2">
                            ...
                            <button :disabled="loading" class="h-9 px-4 rounded-lg text-[13px] font-semibold bg-gray-900 hover:bg-black text-white shadow-sm transition-colors disabled:opacity-40 dark:shadow-none" @click="submitPerms">{{ loading ? 'Saving…' : 'Save permissions' }}</button>
                        </div>
                    </div>
                </div>
            </div>
