...
<template>
    <div style="font-family: var(--wos-font)" class="space-y-6">
        ...
        <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
            <div>
                <h1 class="text-[20px] font-semibold text-gray-900 tracking-tight">Access Control Matrix</h1>
                <p class="text-[13px] text-gray-500 mt-0.5">Map permissions directly to roles using an interactive, real-time matrix.</p>
            </div>
        </div>
                ...
                <thead>
                    <tr class="bg-[#f9fafb] border-b border-gray-200">
                        ...
                        <th 
                            v-for="role in roles" 
                            :key="role.id" 
                            class="px-4 py-3 text-[12px] font-semibold text-gray-800 text-center min-w-[120px] border-r border-gray-100 last:border-r-0"
                        >
                            <div class="flex items-center justify-center gap-1.5">
                                <span>{{ role.nama }}</span>
                                <Loader2 v-if="savingRoleIds.includes(role.id)" class="w-3 h-3 text-gray-500 animate-spin" />
                            </div>
                            <span class="block mt-0.5 text-[10px] font-mono font-normal text-gray-400 uppercase tracking-tight">{{ role.slug }}</span>
                        </th>
                    </tr>
                </thead>
                <tbody class="text-sm">
                    <template v-for="(perms, groupName) in groupedPermissions" :key="groupName">
                        ...
                        <tr class="bg-gray-50/70 border-b border-gray-200">
                            <td 
                                ...
                                class="px-4 py-2 text-[11px] font-bold text-gray-500 uppercase tracking-widest bg-gray-50/70 sticky left-0"
                            >
                                {{ groupName }}
                            </td>
                        </tr>
                        ...
                        <tr 
                            v-for="perm in perms" 
                            :key="perm.id" 
                            class="border-b border-gray-100 hover:bg-[#f9fafb]/40 last:border-b-0 transition-colors"
                        >
                            <!-- Permission info (sticky on scroll) -->
                            <td class="px-4 py-3 sticky left-0 bg-white border-r border-gray-200 shadow-[2px_0_5px_rgba(0,0,0,0.02)] min-w-[320px] whitespace-normal">
                                <div class="pr-2">
                                    <p class="text-[13px] font-semibold text-gray-900 leading-normal">{{ perm.name }}</p>
                                    <p class="font-mono text-[10.5px] text-gray-400 mt-0.5">{{ perm.slug }}</p>
                                    <p v-if="perm.description" class="text-[11.5px] text-gray-400 mt-1 leading-normal">{{ perm.description }}</p>
                                </div>
                            </td>
                            ...
                            <td 
                                v-for="role in roles" 
                                :key="role.id" 
                                class="p-3 text-center border-r border-gray-100 last:border-r-0 align-middle"
                            >
                                <div class="flex items-center justify-center">
                                    <button
                                        ...
                                        :class="[
                                            'w-5 h-5 rounded border flex items-center justify-center transition-all focus:outline-none focus:ring-1 focus:ring-gray-900',
        ...
        <div v-else class="rounded-xl bg-[#f9fafb] border border-dashed border-gray-200 p-12 flex flex-col items-center justify-center text-center gap-2">
            ...
            <p class="text-[13px] font-medium text-gray-800">No results found</p>
            <p class="text-[11.5px] text-gray-400">Try adjusting your search criteria or add new permissions/roles.</p>
        </div>
    </div>
</template>