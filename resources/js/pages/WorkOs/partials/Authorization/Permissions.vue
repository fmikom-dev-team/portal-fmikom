...
<template>
    <div class="p-6 md:p-8">
        ...
        <!-- Empty state -->
        <div v-if="!permissions.length" class="bg-white dark:bg-zinc-900 border border-dashed border-gray-200 dark:border-zinc-700 rounded-xl p-12 text-center">
            <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-zinc-800 flex items-center justify-center mx-auto mb-3">
                <svg class="w-5 h-5 text-gray-400 dark:text-zinc-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.8"><path stroke-linecap="round" stroke-linejoin="round" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>
            </div>
            <p class="text-[14px] font-medium text-gray-700 dark:text-zinc-300 mb-1">No permissions yet</p>
            <p class="text-[12px] text-gray-400 dark:text-zinc-500 mb-4">Create your first permission to control access granularly.</p>
        ...
        <div v-else class="space-y-6">
            <div v-for="(perms, group) in groupedPerms" :key="group">
                <div class="flex items-center gap-2 mb-2">
                    <p class="text-[11px] font-bold text-gray-400 dark:text-zinc-500 uppercase tracking-widest capitalize">{{ group }}</p>
                    <span class="text-[10px] text-gray-400 dark:text-zinc-500 bg-gray-100 dark:bg-zinc-800 px-1.5 py-0.5 rounded tabular-nums">{{ perms.length }}</span>
                </div>
                <div class="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-700 rounded-xl overflow-x-auto shadow-sm dark:shadow-none">
                    <table class="w-full text-left whitespace-nowrap">
                        ...
                        <thead>
                            <tr class="border-b border-gray-100 dark:border-zinc-800 bg-gray-50 dark:bg-zinc-900/80">
                                <th class="px-4 py-2.5 text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Name</th>
                                <th class="px-4 py-2.5 text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Slug</th>
                                <th class="px-4 py-2.5 text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Assigned to</th>
                                <th class="px-4 py-2.5 text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Description</th>
                                <th class="px-4 py-2.5 w-24 text-right text-[11px] font-semibold text-gray-500 dark:text-zinc-400 uppercase tracking-wider">Actions</th>
                            </tr>
                        </thead>
                        <tbody>
                            <tr v-for="p in perms" :key="p.id" class="border-b border-gray-50 last:border-0 hover:bg-gray-50 dark:hover:bg-zinc-800 dark:bg-zinc-900/60 group transition-colors">
                                <td class="px-4 py-3 text-[13px] font-medium text-gray-900 dark:text-zinc-100">{{ p.name }}</td>
                                <td class="px-4 py-3">
                                    <span class="font-mono text-[11px] bg-gray-100 dark:bg-zinc-800 px-2 py-0.5 rounded border border-gray-200 dark:border-zinc-700 text-gray-600 dark:text-zinc-400">{{ p.slug }}</span>
                                </td>
                                <td class="px-4 py-3 text-[13px]">
                                    <span v-if="p.roles_count > 0" class="font-medium text-gray-900 dark:text-zinc-100">{{ p.roles_count }} role{{ p.roles_count !== 1 ? 's' : '' }}</span>
                                    <span v-else class="text-gray-300">None</span>
                                </td>
                                <td class="px-4 py-3 text-[12px] text-gray-400 dark:text-zinc-500">{{ p.description || '—' }}</td>
                                <td class="px-4 py-3 text-right">
                                    <div class="flex items-center justify-end gap-1.5 opacity-0 group-hover:opacity-100 transition-opacity">
                                        <button class="h-6 px-2.5 rounded-md text-[11px] text-gray-600 dark:text-zinc-400 border border-gray-200 dark:border-zinc-700 hover:bg-gray-100 dark:hover:bg-zinc-800 dark:bg-zinc-800 transition-colors" @click="openEdit(p)">Edit</button>
        ...
        <Teleport to="body">
            ...
            <div v-if="modal.createPerm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4" @click.self="modal.createPerm = false">
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-md dark:shadow-none">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        ...
                        <div><label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Group</label><input v-model="permForm.group" type="text" placeholder="e.g. users, roles, modules" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 placeholder:text-gray-300 transition-all"/></div>
            ...
            <div v-if="modal.editPerm" class="fixed inset-0 z-50 flex items-center justify-center bg-black/30 backdrop-blur-[2px] p-4" @click.self="modal.editPerm = false">
                <div class="bg-white dark:bg-zinc-900 rounded-2xl shadow-2xl border border-gray-200 dark:border-zinc-700 w-full max-w-md dark:shadow-none">
                    ...
                    <div class="px-6 py-5 space-y-4">
                        <div><label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Name</label><input v-model="permForm.name" type="text" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all"/></div>
                        <div><label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Slug</label>
                            <div class="flex h-9 border border-gray-300 dark:border-zinc-700 rounded-lg overflow-hidden focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:border-blue-400 transition-all">
                                <span class="px-3 text-[11px] font-mono text-gray-400 dark:text-zinc-500 bg-gray-50 dark:bg-zinc-900 flex items-center border-r border-gray-200 dark:border-zinc-700">slug:</span>
                        ...
                        <div><label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Group</label><input v-model="permForm.group" type="text" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all"/></div>
                        <div><label class="block text-[12px] font-semibold text-gray-700 dark:text-zinc-300 mb-1.5">Description</label><input v-model="permForm.description" type="text" class="w-full h-9 px-3 text-[13px] border border-gray-300 dark:border-zinc-700 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500/20 focus:border-blue-400 transition-all"/></div>
                    </div>
                    <div class="px-6 py-4 border-t border-gray-100 dark:border-zinc-800 flex justify-end gap-2">
