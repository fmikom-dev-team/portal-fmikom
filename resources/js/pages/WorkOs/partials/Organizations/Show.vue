...
<template>
    <div class="px-8 pt-6 pb-12 w-full max-w-[1200px]" style="font-family: var(--wos-font)">
        <!-- Breadcrumb -->
        <div class="flex items-center gap-2 text-sm mb-5">
            <a href="#" class="text-[#2563EB] hover:underline" @click.prevent="emit('back')">Organizations</a>
            <span class="text-[#d1d5db]">/</span>
            <span class="text-[#6b7280]">Organization details</span>
        </div>
        ...
        <div class="flex items-start gap-4 mb-6">
            ...
            <div class="flex-1">
                <div class="flex items-center gap-3 mb-1.5">
                    <h1 class="text-2xl font-semibold text-[#111827] tracking-tight leading-none">{{ organization?.name || 'Test Organization' }}</h1>
                </div>
                <div class="flex items-center gap-2">
                    <span class="inline-flex items-center px-2 py-0.5 rounded text-xs font-mono text-[#4b5563] bg-[#f3f4f6]">
                        {{ organization?.code || 'org_01KMQ8PKS7Q4B5TTQDHS2R4QMW' }}
                    </span>
                    <span class="inline-flex items-center gap-1.5 text-sm text-[#4b5563]">
