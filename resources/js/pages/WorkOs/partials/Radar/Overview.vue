...
<template>
    <div class="p-6 md:p-8">
        <div class="mb-6">
            <h1 class="text-[22px] font-semibold text-gray-900 tracking-tight mb-1">Radar</h1>
            <p class="text-[13px] text-gray-500">Real-time threat detection and security monitoring.</p>
        </div>
        ...
        <div v-if="radarTab === 'Overview'">
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm mb-5">
                    ...
                    <div class="flex items-center justify-between gap-4 flex-wrap">
                        <div class="flex items-start gap-8 flex-wrap">
                            <div v-for="stat in overviewStats" :key="stat.label">
                                <p class="text-[28px] font-bold text-gray-900 tabular-nums">{{ stat.value }}</p>
                                <p class="text-[12px] text-gray-500 mt-0.5">{{ stat.label }}</p>
                            </div>
                        </div>
                        <!-- Reset button -->
                        <button
                            ...
                            class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 text-red-600 text-[12px] font-medium hover:bg-red-50 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"
                        >
                ...
                <div class="border-b border-gray-100 bg-[#f9fafb]/50 px-6 py-5 relative">
                    <div class="h-44 w-full relative" ref="chartContainer" @mousemove="handleMouseMove" @mouseleave="handleMouseLeave">
                        <svg class="w-full h-full overflow-visible" viewBox="0 0 800 160" preserveAspectRatio="none">
                            ...
                            <!-- Grid lines -->
                            <line v-for="i in 4" :key="i" x1="0" :y1="(i * 32)" x2="800" :y2="(i * 32)" stroke="#e5e7eb" stroke-width="0.8" stroke-dasharray="3 3" />
                ...
                <div class="px-6 py-4 grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-4">
                    <div v-for="b in breakdowns" :key="b.l" class="text-center">
                        <p class="text-[20px] font-bold text-gray-900 tabular-nums">{{ b.v }}</p>
                        <p class="text-[10px] text-gray-500 flex items-center justify-center gap-1 mt-0.5 leading-tight">
            ...
            <!-- Identifiers -->
            <p class="text-[14px] font-semibold text-gray-900 mb-3">Identifiers</p>
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <!-- IP Addresses -->
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                    <p class="text-[13px] font-semibold text-gray-800 mb-4">IP Addresses</p>
                    <div v-if="!topIps.length" class="py-10 text-center text-gray-400">
                    ...
                    <div v-else class="space-y-3">
                        <div v-for="ip in topIps" :key="ip.value" class="space-y-1">
                            <div class="flex items-center justify-between text-[12px]">
                                <span class="font-mono text-gray-700 font-medium">{{ ip.value }}</span>
                ...
                <!-- Countries -->
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                    <p class="text-[13px] font-semibold text-gray-800 mb-4">Countries</p>
                    <div v-if="!topCountries.length" class="py-10 text-center text-gray-400">
                    ...
                    <div v-else class="space-y-3">
                        <div v-for="c in topCountries" :key="c.value" class="space-y-1">
                            <div class="flex items-center justify-between text-[12px]">
                                ...
                                <span class="text-gray-500 font-semibold">{{ c.count }} ({{ c.percentage }}%)</span>
                            </div>
                            <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                ...
                <!-- Domains -->
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                    <p class="text-[13px] font-semibold text-gray-800 mb-4">Domains</p>
                    <div v-if="!topDomains.length" class="py-10 text-center text-gray-400">
                        <p class="text-[12px]">No domains detected in the last 24 hours</p>
                    </div>
                    <div v-else class="space-y-3">
                        <div v-for="d in topDomains" :key="d.value" class="space-y-1">
                            <div class="flex items-center justify-between text-[12px]">
                                <span class="font-mono text-gray-700 font-medium">{{ d.value }}</span>
                                <span class="text-gray-500 font-semibold">{{ d.count }} ({{ d.percentage }}%)</span>
                            </div>
                            <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
                ...
                <!-- Users -->
                <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5">
                    <p class="text-[13px] font-semibold text-gray-800 mb-4">Users</p>
                    <div v-if="!topUsers.length" class="py-10 text-center text-gray-400">
                        <p class="text-[12px]">No users detected in the last 24 hours</p>
                    </div>
                    <div v-else class="space-y-3">
                        <div v-for="u in topUsers" :key="u.value" class="space-y-1">
                            <div class="flex items-center justify-between text-[12px]">
                                <span class="text-gray-700 font-medium truncate max-w-[200px]" :title="u.value">{{ u.value }}</span>
                                <span class="text-gray-500 font-semibold shrink-0">{{ u.count }} ({{ u.percentage }}%)</span>
                            </div>
                            <div class="w-full bg-gray-100 h-1.5 rounded-full overflow-hidden">
        ...
        <div v-else-if="radarTab === 'Detections'">
            ...
            <div class="flex flex-wrap items-center gap-2 mb-4">
                ...
                <div class="relative filter-popover-container">
                    ...
                    <!-- Type Popover -->
                    <div v-if="activeDropdown === 'type'" class="absolute left-0 mt-2 w-56 rounded-lg bg-white border border-gray-200 shadow-lg py-2 z-30">
                ...
                <div class="relative filter-popover-container">
                    ...
                    <!-- Action Popover -->
                    <div v-if="activeDropdown === 'action'" class="absolute left-0 mt-2 w-48 rounded-lg bg-white border border-gray-200 shadow-lg py-2 z-30">
                ...
                <div class="relative filter-popover-container">
                    ...
                    <!-- User Popover -->
                    <div v-if="activeDropdown === 'user'" class="absolute left-0 mt-2 w-56 rounded-lg bg-white border border-gray-200 shadow-lg p-2.5 z-30">
                        <div class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                ...
                <div class="relative filter-popover-container">
                    ...
                    <!-- IP Address Popover -->
                    <div v-if="activeDropdown === 'ip'" class="absolute left-0 mt-2 w-56 rounded-lg bg-white border border-gray-200 shadow-lg p-2.5 z-30">
                        <div class="text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1.5 flex items-center justify-between">
                ...
                <div class="relative filter-popover-container">
                    ...
                    <!-- Country Popover -->
                    <div v-if="activeDropdown === 'country'" class="absolute left-0 mt-2 w-56 rounded-lg bg-white border border-gray-200 shadow-lg py-2 z-30 font-sans">
                        <div class="px-3 py-1 text-[11px] font-semibold text-gray-400 uppercase tracking-wider mb-1 flex items-center justify-between">
                ...
                <button v-if="hasActiveFilters" @click="resetFilters" class="text-[11.5px] font-medium text-red-600 hover:text-red-800 transition-colors ml-2">
                    Clear filters
                </button>
            ...
            <div class="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden">
                <div v-if="!filteredDetections?.length" class="py-20 flex flex-col items-center justify-center text-gray-400 bg-white">
                    ...
                    <p class="text-[13px] font-medium text-gray-600 mb-1">No detections</p>
                    <p class="text-[12px] text-gray-400">No activity matches the selected filter options.</p>
                </div>
                ...
                <div v-else class="overflow-x-auto">
                    <table class="w-full text-left whitespace-nowrap">
                        ...
                        <tbody class="divide-y divide-gray-100">
                            <tr v-for="d in filteredDetections" :key="d.id" class="hover:bg-gray-50/50 transition-colors">
                                <td class="px-6 py-3">
                                    <div class="flex items-center gap-2">
                                        <span class="text-[13px] font-medium text-gray-900">{{ d.type }}</span>
                                ...
                                <td class="px-6 py-3">
                                    <span :class="[
                                        ...
                                        d.action === 'Blocked' ? 'bg-red-50 text-red-700 border-red-100' :
                                        d.action === 'Challenged' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                                ...
                                <td class="px-6 py-3">
                                    <span class="text-[13px] text-gray-600 font-medium">{{ d.metadata?.email || d.metadata?.user || 'anonymous' }}</span>
                                </td>
                                <td class="px-6 py-3 font-mono text-[12.5px] text-gray-700">
                                    {{ d.ip }}
                                </td>
                                <td class="px-6 py-3 text-[13px] text-gray-600">
                                    {{ d.country || 'Unknown' }}
                                </td>
                                <td class="px-6 py-3">
                                    <p class="text-[12.5px] text-gray-700 font-medium">{{ d.created_at }}</p>
                                    <p class="text-[10.5px] text-gray-400 mt-0.5">{{ d.created_at_human }}</p>
                                </td>
                                <td class="px-6 py-3 text-right">
                                    <button @click.stop="selectedDetection = d" class="p-1.5 rounded-md hover:bg-gray-100 text-gray-400 hover:text-gray-600 transition-colors" title="View details">
        ...
        <div v-else-if="radarTab === 'Configuration'">
            <Configuration :radarConfig="radarConfig" :radarBlockedItems="radarBlockedItems" />
        </div>
        ...
        >
            <div class="space-y-4" v-if="selectedDetection">
                ...
                <div class="flex items-center justify-between pb-3 border-b border-gray-100">
                    <div>
                        <p class="text-[14px] font-bold text-gray-900">{{ selectedDetection.type }}</p>
                        <p class="text-[11.5px] text-gray-400 mt-0.5">{{ selectedDetection.created_at }} ({{ selectedDetection.created_at_human }})</p>
                    </div>
                    <span :class="['px-2.5 py-1 rounded text-[11px] font-semibold border',
                        selectedDetection.action === 'Blocked' ? 'bg-red-50 text-red-700 border-red-100' :
                        selectedDetection.action === 'Challenged' ? 'bg-amber-50 text-amber-700 border-amber-100' :
                        'bg-emerald-50 text-emerald-700 border-emerald-100'
                    ]">
                ...
                <div class="grid grid-cols-2 gap-4 text-[12px]">
                    <div>
                        <p class="text-gray-400 font-medium">Risk Score</p>
                        <p :class="['text-[14px] font-bold mt-0.5', selectedDetection.risk_score > 80 ? 'text-red-600' : selectedDetection.risk_score > 50 ? 'text-amber-600' : 'text-emerald-600']">
                            {{ selectedDetection.risk_score }} / 100
                        </p>
                    </div>
                    <div>
                        <p class="text-gray-400 font-medium">Severity</p>
                    ...
                    <div>
                        <p class="text-gray-400 font-medium">IP Address</p>
                        <p class="font-mono text-gray-900 font-medium mt-0.5">{{ selectedDetection.ip }}</p>
                    </div>
                    <div>
                    ...
                    <div class="col-span-2">
                        <p class="text-gray-400 font-medium font-sans">Device & OS</p>
                        <p class="text-gray-900 font-medium mt-0.5">{{ selectedDetection.device }}</p>
                    </div>
                </div>

                <!-- Metadata Block -->
                <div class="mt-4 pt-3 border-t border-gray-100" v-if="selectedDetection.metadata">
                    <p class="text-[12px] text-gray-400 font-medium mb-1.5">Metadata</p>
                    <pre class="bg-gray-50 border border-gray-100 rounded-lg p-3 text-[11px] font-mono text-gray-700 overflow-x-auto max-h-48 leading-relaxed">{{ JSON.stringify(selectedDetection.metadata, null, 2) }}</pre>
                </div>
            </div>
        ...
        <AppModal :show="showResetConfirm" title="Reset all detections" @close="showResetConfirm = false">
            <div class="flex flex-col items-center text-center py-2">
                ...
                <p class="text-[14px] text-gray-700 leading-relaxed max-w-xs">
            ...
            <template #footer>
                ...
                <button
                    class="h-[34px] px-4 rounded-md text-[13px] font-semibold text-white bg-red-600 hover:bg-red-700 transition-colors shadow-sm"
                    @click="confirmReset"
                >
                    Yes, clear all
                </button>
