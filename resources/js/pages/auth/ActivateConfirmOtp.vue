...
const submit = () => {
	form.post("/activate/confirm/otp");
};

const resendOtp = () => {
	resendForm.post("/activate/confirm/resend-otp");
};
</script>

<template>
    <AuthLayout title="Verifikasi OTP Aktivasi" description="Masukkan kode verifikasi yang dikirim ke email Anda">
        <Head>
        ...
        <div class="text-center mb-6">
            ...
            <p class="text-sm text-slate-500 dark:text-slate-400">
                Kode verifikasi OTP 6 digit telah dikirimkan ke email
            </p>
            <p class="text-sm font-semibold text-[#2563eb] dark:text-blue-400 mt-0.5">{{ email }}</p>
        </div>

        <!-- Status Message -->
        <div v-if="status" class="mb-4 text-center text-sm font-medium text-green-700 dark:text-green-300 bg-green-50 dark:bg-green-950/20 p-3 rounded-xl border border-green-200 dark:border-green-900 flex items-center gap-2 justify-center">
            ...
            <div class="grid gap-6">
                    ...
                    <div class="flex justify-center gap-2.5" @paste="handleOtpPaste">
                        <input
                            ...
                            class="w-12 h-14 text-center text-xl font-bold border-2 rounded-xl outline-none transition-all duration-200 bg-white dark:bg-slate-950
                                   border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 placeholder-slate-200
                                   focus:border-[#2563eb] focus:bg-indigo-50/30 focus:ring-2 focus:ring-[#2563eb]/20"
                            :class="otpDigits[i] ? 'border-[#2563eb] bg-indigo-50/20 dark:bg-indigo-950/30' : ''"
                        />
                    </div>
                ...
                <div v-if="expiresAt" class="text-center">
                    ...
                    <p v-else class="text-xs text-red-500 font-medium">
                        Kode OTP sudah kadaluarsa. Silakan kirim ulang di bawah.
                    </p>
                </div>
                ...
                <div class="flex flex-col gap-3">
                    ...
                    <Button
                        ...
                        @click="resendOtp"
                        variant="ghost"
                        class="w-full text-slate-500 dark:text-slate-450 hover:text-[#2563eb] dark:hover:text-blue-400 h-10 transition-colors text-sm font-medium"
