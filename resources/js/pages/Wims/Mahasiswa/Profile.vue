...
// ─── FITUR 5: QUICK ACTIONS ───────────────────────────────────────────────────

const quickActions = computed(() => {
    const status = props.registration?.status;
    const actions: { label: string; href: string; icon: string; desc: string; color: string }[] = [];
    if (!status) {
        actions.push({ label: 'Daftar Magang', href: '/wims/pendaftaran', icon: 'Building2', desc: 'Ajukan pendaftaran PKL baru', color: 'text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-500/15 border-blue-200 hover:bg-blue-100 dark:bg-blue-500/20' });
    }
    if (status === 'aktif' || status === 'selesai') {
        actions.push({ label: 'Logbook Harian', href: '/wims/logbook', icon: 'BookOpen', desc: 'Isi logbook harian magang', color: 'text-emerald-600 dark:text-emerald-400 bg-emerald-50 dark:bg-emerald-500/15 border-emerald-200 hover:bg-emerald-100 dark:bg-emerald-500/20' });
        actions.push({ label: 'Laporan Akhir', href: '/wims/laporan', icon: 'FileText', desc: 'Upload laporan akhir magang', color: 'text-violet-600 dark:text-violet-400 bg-violet-50 dark:bg-violet-500/15 border-violet-200 hover:bg-violet-100 dark:bg-violet-500/20' });
    }
    if (status === 'revisi') {
        actions.push({ label: 'Revisi Pengajuan', href: '/wims/pendaftaran/revisi', icon: 'AlertCircle', desc: 'Perbaiki pengajuan yang diminta revisi', color: 'text-amber-600 dark:text-amber-400 bg-amber-50 dark:bg-amber-500/15 border-amber-200 hover:bg-amber-100 dark:bg-amber-500/20' });
    }
    actions.push({ label: 'Panduan WIMS', href: '/wims/panduan', icon: 'Info', desc: 'Pelajari cara menggunakan WIMS', color: 'text-slate-600 dark:text-slate-400 bg-slate-50 dark:bg-slate-800/50 border-wims-border hover:bg-slate-100 dark:hover:bg-slate-700/40 dark:bg-slate-700/50' });
    return actions;
});

// ─── FITUR 6: DRAG & DROP PHOTO ──────────────────────────────────────────────
    ...
    <div class="min-h-full bg-wims-bg">
        <div class="space-y-5 lg:space-y-6">
            ...
            <div class="grid gap-5 lg:grid-cols-2">
                ...
                <!-- ════════════════════════════════════════════════════════════
                     FITUR 5 — QUICK ACTIONS
                     Panel pintasan navigasi kontekstual. Aksi yang ditampilkan
                     menyesuaikan status magang mahasiswa secara otomatis.
                ═════════════════════════════════════════════════════════════ -->
                <Card class="rounded-xl border border-wims-border bg-wims-card py-0 shadow-[0_18px_36px_-30px_rgba(15,23,42,0.18)]">
                    <CardHeader class="border-b border-wims-border/80 px-5 pt-5 pb-4 sm:px-6 sm:pt-6">
                        <CardTitle class="flex items-center gap-2 text-base text-slate-950 dark:text-white">
                        ...
                        <CardDescription class="mt-0.5 text-sm text-slate-500 dark:text-slate-400">
                            Pintasan ke halaman yang paling sering digunakan.
                        </CardDescription>
                    </CardHeader>
                    <CardContent class="px-5 py-5 sm:px-6">
                        <div class="grid gap-2">
                            ...
                            >
                                <div class="flex items-center gap-3">
                                    <Building2 v-if="action.icon === 'Building2'" class="size-4 shrink-0" />
                                    <BookOpen v-else-if="action.icon === 'BookOpen'" class="size-4 shrink-0" />
                                    <FileText v-else-if="action.icon === 'FileText'" class="size-4 shrink-0" />
                                    <AlertCircle v-else-if="action.icon === 'AlertCircle'" class="size-4 shrink-0" />
