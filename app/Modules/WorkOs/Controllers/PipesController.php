...
use App\Models\Pipes\PipeWorkflow;
...
class PipesController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        $stats = [
            ...
            'failed_syncs' => PipeSyncLog::where('status', 'failed')->where('created_at', '>=', now()->subDays(7))->count(),
     ...
     */
    public function syncConnection(PipeConnection $connection, SyncEngine $syncEngine)
    {
        try {
    ...
    {
        $workflows = PipeWorkflow::with(['organization', 'triggers', 'actions'])
            ->orderBy('created_at', 'desc')
            ->paginate(20);
