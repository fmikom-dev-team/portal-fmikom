...
class FinalReportTemplateAccessService
{
    ...
    private const DIRECTORY = 'template-pkl';
    ...
    public function storeTemplateFile(UploadedFile $file): string
    {
        ...
        if ($contents === false) {
            throw new RuntimeException('Gagal membaca file template PKL yang diunggah.');
        }

        if (! Storage::disk(self::DISK)->put($path, $contents)) {
            throw new RuntimeException('Gagal menyimpan template PKL.');
        }
    ...
    {
        $fallbackName = $template->template_type === 'proposal' ? 'template-proposal-pkl' : 'template-laporan-akhir';
        $safeName = str($template->original_name ?: $template->title ?: $fallbackName)
            ->replace(['../', '..\\', '"', "\r", "\n"], '')
            ->value();
    ...
    public function resolveAbsolutePath(FinalReportTemplate $template): string
    {
        ...
        abort_unless($location !== null, 404, 'File template PKL tidak ditemukan.');
