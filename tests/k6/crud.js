...
export default function () {
    ...
    const createRes = http.post(`${TARGET_URL}/portal-admin/categories`, createPayload, { headers });
    ...
    const readRes = http.get(`${TARGET_URL}/portal-admin/categories`, { headers });
    check(readRes, {
    ...
    const updateRes = http.put(`${TARGET_URL}/portal-admin/categories/${categoryId}`, updatePayload, { headers });
    check(updateRes, {
        'update category status is 200 or 302': (r) => r.status === 200 || r.status === 302,
    });

    sleep(1);

    // 4. DELETE (Destroy category to clean up db)
    const deleteRes = http.del(`${TARGET_URL}/portal-admin/categories/${categoryId}`, null, { headers });
    check(deleteRes, {
        'delete category status is 200 or 302': (r) => r.status === 200 || r.status === 302,
    });
