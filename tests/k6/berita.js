...
export default function () {
	...
	const links = [];
	doc.find("a").each((_idx, el) => {
		const href = el.getAttribute("href");
		if (href?.includes("/berita/") && href !== "/berita") {
			links.push(href);
		}
	});
	...
	if (links.length > 0) {
		...
		const articleUrl = randomLink.startsWith("http")
			? randomLink
			: `${TARGET_URL}${randomLink}`;
		...
		check(fallbackRes, {
			"fallback article status is 200 or 404": (r) =>
