...
test.describe("Authentication — Forgot Password", () => {
	...
	test("halaman forgot password dapat diakses", async ({ page }) => {
		...
		await expect(
			page.locator(FORGOT_PASSWORD_SELECTORS.submitButton),
		).toBeVisible();
	});

	test("menampilkan pesan sukses setelah submit email valid", async ({
		page,
		db,
	}) => {
		await db.seed();
		...
		const statusEl = page
			.locator('.text-green-600, [class*="success"]')
			.first();
	...
	test('link "kembali ke login" berfungsi', async ({ page }) => {
		await page.click(
			'a:has-text("log in"), a:has-text("Login"), a:has-text("login")',
		);
