...
test.describe("Admin — Approve User Registration", () => {
	...
	test("admin dapat melihat daftar pending users di WorkOs", async ({
		page,
	}) => {
	...
	test("approve API endpoint tersedia dan dapat diakses", async ({
		page,
		db,
	}) => {
		await db.seed();
	...
	test("admin dapat melihat detail pending registration request", async ({
		page,
	}) => {
		await page.goto(ROUTES.workosUsers);
		await page.waitForLoadState("networkidle");
		...
		const rows = page.locator(
			'table tbody tr, [class*="user-row"], [class*="list-item"]',
		);
