...
export class DashboardPage {
	...
	async getModuleCards(): Promise<Locator[]> {
		...
		const cards = this.page.locator(
			'[data-test="module-card"], .module-card, [class*="module"]',
		);
	...
	async logout(): Promise<void> {
		...
		const logoutBtn = this.page
			.locator(
				'[data-test="logout-button"], button:has-text("Logout"), button:has-text("Keluar"), a:has-text("Logout"), a:has-text("Keluar")',
			)
			.first();
