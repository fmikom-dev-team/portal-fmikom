...
export class WorkOsDashboardPage {
    ...
    async searchUser(query: string): Promise<void> {
        const searchInput = this.page
            .locator('[data-test="search-input"], input[placeholder*="Search"], input[placeholder*="Cari"]')
            .first();
    ...
    async findPendingUserRow(): Promise<Locator | null> {
        const pendingRows = this.page.locator('tr:has-text("pending"), [data-status="pending"], [class*="pending"]');
    ...
    async getUserCount(): Promise<number | null> {
        try {
            const countEl = this.page.locator('[data-test="user-count"], .stat-users').first();
