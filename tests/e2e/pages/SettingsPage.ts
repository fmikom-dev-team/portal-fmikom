...
export class SettingsPage {
    ...
    async gotoSecurity(password: string = 'Playwright@Mahasiswa1'): Promise<void> {
        ...
        if (this.page.url().includes('confirm-password')) {
            ...
            const confirmBtn = this.page.locator('button:has-text("Confirm"), button:has-text("Konfirmasi")').first();
    ...
    async updateName(name: string): Promise<void> {
        ...
        const submitBtn = this.page.locator('button[type="submit"]:has-text("Save"), button:has-text("Simpan")').first();
    ...
    async getFlashMessage(): Promise<string> {
        const flash = this.page.locator('[data-test="flash"], .toast, [class*="success"], [class*="alert"]').first();
